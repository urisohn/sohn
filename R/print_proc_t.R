#' Print method for proc_t output
#'
#' @param x An object of class \code{proc_t}
#' @param ... Additional arguments passed to print
#'
#' @export
print.proc_t <- function(x, ...) {
  # Ensure x is a data frame
  if (!is.data.frame(x)) {
    # Remove the proc_t class temporarily to avoid infinite recursion
    class(x) <- setdiff(class(x), "proc_t")
    return(print(x, ...))
  }
  ...
  # Print the dataframe
  # Remove the proc_t class temporarily to avoid infinite recursion
  class(display_x) <- setdiff(class(display_x), "proc_t")
